    ...
    module PasswordArchivable
      ...
      def password_archive_included?
        ...
        if self.class.deny_old_passwords > 0 && !self.password.nil?
          old_passwords_including_cur_change = self.old_passwords.order(created_at: :desc).limit(self.class.deny_old_passwords).to_a
      ...
      def archive_password
        if encrypted_password_changed?
          if archive_count.to_i > 0
            ...
            old_passwords.order(created_at: :desc).offset(archive_count).destroy_all
          else
